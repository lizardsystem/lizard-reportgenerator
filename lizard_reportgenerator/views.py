...
def index(request, template='lizard_reportgenerator/index.html'):
    ...
    if request.user.is_superuser:
        ...
        data_sets = DataSet.objects.filter(
            permission_mappers__user_group__members=request.user).distinct()
    ...
    report_bloks.append({
        ...
        'generated_reports': GeneratedReport.objects.filter(template__kind='krw',
                                                            area=krw_gebied)
    })
    ...
    report_bloks.append({
        ...
        'generated_reports': GeneratedReport.objects.filter(template__kind='aan_afvoer',
                                                            area=aan_afvoergebied)
    })
...
def generate_report(request_or_username, file_format='pdf', report_id=None, area_id=None, return_as_file=False):
    '''
    ...
    report_module = __import__(report_template.generation_module, fromlist='something')
    filename = "{0}_{1}.{2}".format(report_template.slug, area_id, file_format)
    ...
    if file_format == 'pdf':
        ...
        if return_as_file:
            ...
            response = HttpResponse(report, mimetype='application/{}'.format(file_format))
        ...
        grid = getattr(report_module, report_template.generation_function)(username, file_format='grid', report_id=report_id, area_id=area_id)
...
def get_generated_report(request, generated_report_id=None, file_format='pdf'):
    ...
    response = HttpResponse(generated_report.get_file(file_format),
                            mimetype='application/' + file_format)
